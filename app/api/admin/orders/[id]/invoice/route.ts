...
export async function GET(
  ...
  const displayNum = order.orderCode ?? order.orderNumber ?? order.id;
  const invoiceNo  = order.orderCode
    ? `INV-${year}-${order.orderCode}`
    : `INV-${year}-${String(displayNum).padStart(5, '0')}`;

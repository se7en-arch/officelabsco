  ...
  <tr>
    <td style="background:#fff;padding:28px 32px">
      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #eee;border-radius:6px;overflow:hidden;border-collapse:collapse">
        ...
        <tr style="background:#f9f9f9">
          <td colspan="3" style="padding:4px 14px;font-size:11px;color:#888;text-align:right">ДДС</td>
          <td style="padding:4px 14px;font-size:13px;color:#1a1a1a;text-align:right">0</td>
        </tr>
...
export async function sendOrderNotification(order: OrderEmailData): Promise<void> {
  if (!process.env.RESEND_API_KEY) {
    console.warn('[Mailer] RESEND_API_KEY не е зададен — пропускам изпращането на имейл.');
    return;
  }
  ...
  try {
    ...
    if (error) {
      console.error('[Mailer] Грешка от Resend:', error);
    } else {
      console.log(`[Mailer] Имейл изпратен до ${to} за поръчка #${order.id}`);
    }
  } catch (err) {
    console.error('[Mailer] Изключение при изпращане:', err);
  }
}
